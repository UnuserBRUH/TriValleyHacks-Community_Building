...
onready var robux_generator = $money_maker
...
func _unhandled_input(event):
	if event is InputEventMouseButton && event.pressed && event.button_index == BUTTON_LEFT &&  $Area2D.get_overlapping_areas().size() <= 0:
		...
		functionality()
		$Status.hide()
		Global.factory_amount += 1
		set_process(false)
		set_process_unhandled_input(false)

func functionality():
	robux_generator.start(5)



func _on_money_maker_timeout():
	Global.money_current += 50 + round((Global.population_current*1.5)) + round(Global.factory_amount * 1.5)
	robux_generator.start(5)
